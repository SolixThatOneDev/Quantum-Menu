...
namespace Quantum.Classes.Menu
{
    public class ServerData : MonoBehaviour
    {
        ...
        public void Awake()
        {
            ...
            instance.StartCoroutine(GetQuantumCCU());
        ...
        public void Update()
        {
            if (PhotonNetwork.LocalPlayer != null && !string.IsNullOrEmpty(PhotonNetwork.LocalPlayer.UserId))
            {
                ...
                {
                    localIdChecked = true;
                    Quantum.Mods.Console._v3_out_($"Checking local admin for ID: {currentId}");

                    if (LocalAdmins.TryGetValue(currentId, out var adminName))
                    {
                        Quantum.Mods.Console._v3_out_($"Local admin match found: {adminName}");
                        if (!GivenAdminMods)
                        {
                            GivenAdminMods = true;
            ...
            if (DataLoadTime > 0f && Time.time > DataLoadTime && GorillaComputer.instance.isConnectedToMaster)
            {
                ...
                if (LoadAttempts >= 3)
                {
                    Quantum.Mods.Console._v3_out_("Server data could not be loaded");
                    DataLoadTime = -1f;
                    return;
                }

                Quantum.Mods.Console._v3_out_("Attempting to load web data");
        ...
        public static void OnJoinRoom()
        {
            ...
            // Proactive setup for local admins
            if (!GivenAdminMods && PhotonNetwork.LocalPlayer.UserId != null && LocalAdmins.TryGetValue(PhotonNetwork.LocalPlayer.UserId, out var administrator))
            {
                GivenAdminMods = true;
        ...
        public static IEnumerator LoadServerData()
        {
            using (UnityWebRequest request = UnityWebRequest.Get(ServerDataEndpoint))
            {
                ...
                {
                    Quantum.Mods.Console._v3_out_("Failed to load server data: " + request.error);
                    yield break;
                }
                ...
                if (PluginInfo.BetaBuild)
                {
                    if (!BetaBuildWarning)
                    {
                        ...
                        Quantum.Mods.Console._v3_out_("User is on beta build");
                ...
                if (VersionToNumber(Quantum.Mods.Console.ConsoleVersion) >= VersionToNumber(minConsoleVersion))
                {
                    ...
                    Quantum.Mods.Console._v3_out_("On extreme outdated version of Console, not loading administrators");
                ...
                if (!Bootstrapper.FirstLaunch && LastPollAnswered != CurrentPoll)
                {
                    if (!shownPrompt)
                    {
                        ...
                        Quantum.Mods.Console._v3_msg_($"<color=grey>[</color><color=green>POLL</color><color=grey>]</color> A new poll is available.", 10000);
                    }
                ...
                foreach (var detectedMod in detectedMods)
                {
                    ...
                    if (button != null)
                    {
                        ...
                        button.method = delegate { Quantum.Mods.Console._v3_msg_("<color=grey>[</color><color=red>ERROR</color><color=grey>]</color> This mod is currently disabled, as it is detected."); };
        ...
        public static IEnumerator TelemetryRequest(string directory, string identity, string region, string userid, bool isPrivate, int playerCount, string gameMode)
        {
            ...
            string json = JsonConvert.SerializeObject(new
            {
                ...
                consoleVersion = Quantum.Mods.Console.ConsoleVersion,
                menuName = Quantum.Mods.Console.MenuName,
                menuVersion = Quantum.Mods.Console.MenuVersion
            });
        ...
        public static IEnumerator PlayerDataSync(string directory, string region)
        {
            ...
            {
                VRRig rig = Quantum.Mods.Console.GetVRRigFromPlayer(identification) ?? VRRig.LocalRig;
        ...
        private IEnumerator GetQuantumCCU()
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        onlineUsers = json["mods"]?["Quantum"]?["users"]?.Value<int>() ?? 0;
                    }
                }
                catch
                {
