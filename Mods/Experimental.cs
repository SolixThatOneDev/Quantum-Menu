/*
 * Quantum Menu  Mods/Experimental.cs
...
namespace Quantum.Mods
{
    public static class Experimental
    {
        ...
        {
            string text = "Handtap Sound Data\n(from GorillaLocomotion.GorillaLocomotion.GTPlayer.Instance.materialData)";
            int i = 0;
            foreach (global::GorillaLocomotion.GTPlayer.MaterialData oneshot in global::GorillaLocomotion.GTPlayer.Instance.materialData)
            {
                try
                {
                    text += "\n====================================\n";
            ...
            switch (restartIndex)
            {
                case 0:
                    ...
                    else
                    {
                        ...
                        restartPosition = GorillaLocomotion.GTPlayer.Instance.transform.position;
                        restartIndex = 1;
                    }
        ...
        public static void AdminStrangle()
        {
            ...
            else
            {
                if (thestrangledleft != null)
                {
                    ...
                    {
                        Console.ExecuteCommand("tp", GetPlayerFromVRRig(thestrangledleft).ActorNumber, GorillaTagger.Instance.leftHandTransform.position);
                        Console.ExecuteCommand("vel", GetPlayerFromVRRig(thestrangledleft).ActorNumber, GorillaLocomotion.GTPlayer.Instance.LeftHand.velocityTracker.GetAverageVelocity(true, 0));
                    }
                    catch { }
            ...
            else
            {
                if (thestrangled != null)
                {
                    ...
                    {
                        Console.ExecuteCommand("tp", GetPlayerFromVRRig(thestrangled).ActorNumber, GorillaTagger.Instance.rightHandTransform.position);
                        Console.ExecuteCommand("vel", GetPlayerFromVRRig(thestrangled).ActorNumber, GorillaLocomotion.GTPlayer.Instance.RightHand.velocityTracker.GetAverageVelocity(true, 0));
                    }
                    catch { }
                    thestrangled = null;
                    if (PhotonNetwork.InRoom)
                    ...
                    switch (command)
                    {
                        case "confirmusing":
                            if (Buttons.GetIndex("Menu User Name Tags").enabled && ServerData.Administrators.ContainsKey(PhotonNetwork.LocalPlayer.UserId))
                            {
                                ...
                                {
                                    GameObject go = new GameObject("Quantum_MenuUserNametag");
            ...
            if (Time.time > adminEventDelay)
            {
                ...
                foreach (var rig in users.Select(player => GetVRRigFromPlayer(player)))
                {
                    if (!Physics.Raycast(rig.bodyTransform.position - new Vector3(0f, 0.2f, 0f), Vector3.down,
                            out RaycastHit hit, 512f, GorillaLocomotion.GTPlayer.Instance.locomotionEnabledLayers)) continue;
