...
namespace Quantum.Mods
{
    public static class Advantages
    {
        ...
        public static void TagAura()
        {
            ...
            foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig => VRRig.LocalRig.IsTagged() && !vrrig.IsTagged() && !GorillaLocomotion.GTPlayer.Instance.disableMovement && Vector3.Distance(vrrig.headMesh.transform.position, GorillaTagger.Instance.bodyCollider.transform.position) < tagAuraDistance))
            {
                color = Color.green;
                ReportTag(vrrig);
            }
        ...
        public static void FlickTagGun()
        {
            if (GetGunInput(false))
            {
                ...
                if (GetGunInput(true))
                {
                    GorillaLocomotion.GTPlayer.Instance.GetControllerTransform(false).position = NewPointer.transform.position;

                    if (Vector3.Distance(GorillaLocomotion.GTPlayer.Instance.GetControllerTransform(false).position, GorillaTagger.Instance.bodyCollider.transform.position) > 4f)
                        GorillaLocomotion.GTPlayer.Instance.GetControllerTransform(false).position = GorillaTagger.Instance.bodyCollider.transform.position + (GorillaLocomotion.GTPlayer.Instance.GetControllerTransform(false).position - GorillaTagger.Instance.bodyCollider.transform.position) * 4f;
                }
            }
        }
