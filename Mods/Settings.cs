...
namespace Quantum.Mods
{
    ...
    {
        public static List<string> textureFileDirectory = new List<string>();
        ...
        public static void SpawnKeyboard()
        {
            ...
            if (!isKeyboardPc)
            {
                if (VRKeyboard == null)
                {
                    ...
                    VRKeyboard.transform.localScale *= scaleWithPlayer ? GorillaLocomotion.GTPlayer.Instance.scale * menuScale : menuScale;
                    menuSpawnPosition.transform.localScale *= scaleWithPlayer ? GorillaLocomotion.GTPlayer.Instance.scale * menuScale : menuScale;
        ...
        public static void MergePreferences_iisStupidMenu()
        {
            ...
            destination = Path.Combine(PluginInfo.BaseDirectory, "Quantum_Preferences.txt");
        ...
        {
            string fileText = File.ReadAllText($"{PluginInfo.BaseDirectory}/Quantum_Preferences.txt").Replace("\r", "");
            ...
            File.WriteAllText($"{PluginInfo.BaseDirectory}/Quantum_Preferences.txt", string.Join("\n", textData));
        }
        ...
        public static void UpdateTutorial()
        {
            ...
            {
                TutorialSelector = new GameObject("Quantum_TutorialSelector").AddComponent<LineRenderer>();
        ...
        public static void ShowDebug()
        {
            ...
            Buttons.AddButton(category, new ButtonInfo { buttonText = "DebugMenuName", overlapText = "<color=grey><b>Quantum Menu </b></color>" + version, label = true });
        ...
        {
            GameObject cameraObject = new GameObject("Quantum_SpectateCamera");
        ...
        public static void UpdateMenu()
        {
            switch (SystemInfo.operatingSystemFamily)
            {
                case OperatingSystemFamily.Windows:
                    ...
                    dir ""%PLUGIN_PATH%\*Quantum_AutoUpdater*.dll"" >nul 2>&1
                    if %ERRORLEVEL%==0 (
                        goto restart
                    )

                    for %%F in (""%PLUGIN_PATH%\*Quantum*menu*.dll"") do (
                        set ""MENU_FILE=%%F""
                        goto update
                    )
                    ...
                    :update
                    echo Downloading latest release of Quantum Menu...

                    curl -L -o ""%MENU_FILE%"" ^
                    ""https://github.com/Quantum/Quantum-Menu/releases/latest/download/Quantum-Menu.dll""
        ...
        {
            if (!File.Exists($"{PluginInfo.BaseDirectory}/Quantum_CustomThemeColor.txt"))
                WriteCustomTheme();
        ...
        {
            string[] linesplit = File.ReadAllText($"{PluginInfo.BaseDirectory}/Quantum_CustomThemeColor.txt").Split("\n");
        ...
        {
            File.WriteAllText($"{PluginInfo.BaseDirectory}/Quantum_CustomThemeColor.txt", theme);
            ReadCustomTheme();
        }
        ...
        public static void WriteCustomTheme() =>
            File.WriteAllText($"{PluginInfo.BaseDirectory}/Quantum_CustomThemeColor.txt", ExportCustomTheme());
        ...
        public static void EnableWatermark()
        {
            ...
            if (watermarkImage != null)
                watermarkImage.gameObject.SetActive(!disableWatermark);
        }

        public static void TogglePremiumWatermark()
        {
            premiumWatermark = !premiumWatermark;
        }

        public static void CustomWatermark()
        {
            if (!File.Exists($"{PluginInfo.BaseDirectory}/CustomWatermark.png"))
                LoadTextureFromURL($"{PluginInfo.ServerResourcePath}/Images/CustomWatermark.png", "CustomWatermark.png");

            textureFileDirectory.Remove("CustomWatermark.png");
            customWatermark = LoadTextureFromFile("CustomWatermark.png");

            // Update existing watermarks immediately
            if (watermarkImage != null && watermarkImage.material != null)
            {
        ...
        {
            doCustomName = true;
            if (!File.Exists($"{PluginInfo.BaseDirectory}/Quantum_CustomMenuName.txt"))
                File.WriteAllText($"{PluginInfo.BaseDirectory}/Quantum_CustomMenuName.txt", "Your Text Here");

            customMenuName = File.ReadAllText($"{PluginInfo.BaseDirectory}/Quantum_CustomMenuName.txt");
        }
        ...
        private static string[] keyWords = { "jarvis", "Quantum", "seralith", "sarolith", "siri", "google", "alexa", "dummy", "computer", "stinky", "silly", "stupid", "console", "go go gadget", "monika", "wikipedia", "gideon", "a i", "ai", "a.i", "chat gpt", "chatgpt", "grok", "grock", "groq", "garmin" };
        ...
        {
            if (!File.Exists($"{PluginInfo.BaseDirectory}/Quantum_Keywords.txt"))
                File.WriteAllLines($"{PluginInfo.BaseDirectory}/Quantum_Keywords.txt", keyWords);
            keyWords = File.ReadAllLines($"{PluginInfo.BaseDirectory}/Quantum_Keywords.txt");
        ...
        public static void ClickGUI()
        {
            ...
            else
            {
                ...
                if (clickGuiLine == null)
                {
                    clickGuiLine = new GameObject("Quantum_ClickGUILine")
                        .GetOrAddComponent<LineRenderer>();

                    clickGuiLine.material = new Material(Shader.Find("GUI/Text Shader"));
                    clickGuiLine.startWidth = 0.025f * (scaleWithPlayer ? GorillaLocomotion.GTPlayer.Instance.scale : 1f);
            ...
            if (XRSettings.isDeviceActive)
            {
                ...
                if (canSelect)
                {
                    if (selectObject == null)
                        selectObject = new GameObject("Quantum_PingLine");
                    ...
                    pingLine.startWidth = 0.025f * (scaleWithPlayer ? GorillaLocomotion.GTPlayer.Instance.scale : 1f);
                    pingLine.endWidth = 0.025f * (scaleWithPlayer ? GorillaLocomotion.GTPlayer.Instance.scale : 1f);
                    ...
                    Physics.SphereCast(StartPosition + Direction / 4f * (scaleWithPlayer ? GorillaLocomotion.GTPlayer.Instance.scale : 1f), 0.15f, Direction, out var Ray, 512f, NoInvisLayerMask());
        ...
        {
            if (!File.Exists($"{PluginInfo.BaseDirectory}/Quantum_SystemPrompt.txt"))
                File.WriteAllText($"{PluginInfo.BaseDirectory}/Quantum_SystemPrompt.txt", AIManager.SystemPrompt);
        }
        ...
        public static void SavePreferences() =>
            File.WriteAllText($"{PluginInfo.BaseDirectory}/Quantum_Preferences.txt", SavePreferencesToText());
        ...
        {
            string fileName = $"{PluginInfo.BaseDirectory}/Quantum_PCControls.txt";
