/*
 * Quantum Menu  Patches/Menu/WatcherEyesPatch.cs
...
namespace Quantum.Patches.Menu
{
    ...
    public class WatcherEyesPatch
    {
        ...
        public static bool Prefix(HalloweenWatcherEyes __instance)
        {
            if (enabled)
            {
                Quaternion normalized = Quaternion.LookRotation((GorillaLocomotion.GTPlayer.Instance.headCollider.transform.position - __instance.transform.position).normalized);
