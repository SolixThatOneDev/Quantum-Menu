/*
 * Quantum Menu  Patches/Menu/ReleasePatch.cs
        ...
        public static bool Prefix(TakeMyHand_HandLink __instance, bool __result, DropZone zoneReleased, GameObject releasingHand)
        {
            if (enabled)
            {
                if (!__instance.myRig.isOfflineVRRig)
                {
                    ...
                    HandLinkAuthorityStatus selfHandLinkAuthority = GorillaLocomotion.GTPlayer.Instance.TakeMyHand_GetSelfHandLinkAuthority();
                    ...
                    if (grounded)
                    {
                        Vector3 averageVelocity = (handLink.isLeftHand ? GorillaLocomotion.GTPlayer.Instance.LeftHand.velocityTracker : GorillaLocomotion.GTPlayer.Instance.RightHand.velocityTracker).GetAverageVelocity(true).normalized * 20f;
