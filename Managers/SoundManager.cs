/*
 * Quantum Menu  Managers/SoundManager.cs
    ...
    public class SoundManager
    {
        public static readonly Dictionary<string, Dictionary<string, object>> Sounds = new Dictionary<string, Dictionary<string, object>>
        {
            ["Buttons"] = new Dictionary<string, object>
            {
                ...
                { "Quantum", "Audio/Menu/Buttons/sensation.ogg" }
            },
        ...
        private static float soundCooldown;
        ...
        public static void Play(string sound, string outputPath = null, Action<AudioClip> action = null, string buttonText = null, bool overlapHand = false, bool leftOverlap = false)
        {
            ...
            try
            {
                ...
                if (doButtonsVibrate && GorillaTagger.Instance != null)
                ...
                {
                    case int rpcId:
                        VRRig rig = VRRig.LocalRig ?? (GorillaTagger.Instance != null ? GorillaTagger.Instance.offlineVRRig : null);
                        if (rig != null)
                        {
                            // Ensure the hand's audio source is actually audible
                            AudioSource handSource = rightHand ? rig.rightHandPlayer : rig.leftHandPlayer;
                            if (handSource != null)
                            {
                                handSource.volume = 1f;
                                handSource.mute = false;
                            }

                            // Use a very high multiplier (2.0f) to ensure it's heard
                            rig.PlayHandTapLocal(rpcId, rightHand, 2f);

                            /*
                            if (PhotonNetwork.InRoom && serversidedButtonSounds && GorillaTagger.Instance != null && GorillaTagger.Instance.myVRRig != null)
                            {
                                GorillaTagger.Instance.myVRRig.SendRPC("RPC_PlayHandTap", RpcTarget.Others, rpcId, rightHand, 1f);
                            }
                            */
                        }
                        break;
                    ...
                    case string s:
                        ...
                        VRRig rigS = VRRig.LocalRig ?? (GorillaTagger.Instance != null ? GorillaTagger.Instance.offlineVRRig : null);
                        if (rigS != null)
                        {
                            AudioSource audioSource = rightHand ? rigS.rightHandPlayer : rigS.leftHandPlayer;
                            if (audioSource != null)
                            {
                                audioSource.volume = buttonClickVolume / 10f;
                                /* DISABLED FOR STABILITY IN STUMP
            ...
            {
                LogManager.LogError("SoundManager: " + ex.Message);
            }
        }
        ...
        private static object ResolveSoundPath(string sound, string buttonText = null)
        {
            ...
            if (!(rawPath is string s)) return rawPath;
